...
3) **Insulin dose / medication dose / weight / anything that is NOT A1c**
   - Ignore any % clearly tied to:
       ...
       - "body weight", "weight", "kg", "BMI"
   - Example:
       "Patients with a baseline A1c of 8.0% or lower required a larger decrease in insulin
        (-22.6% vs ...)."
     → -22.6% is a change in insulin, NOT A1c → DO NOT extract anything.
...
- When you see a phrase like "HbA1c decreased from X% to Y%" or "reduced from X% to Y%":
  ...
  2) Compute the relative reduction using the BASELINE X:
       relative_reduction = ((X - Y) / X) * 100
     (Use X in the denominator, NOT Y.)
  3) Normalize and round to a reasonable number of decimals (e.g., "25.0%").
  4) Add this relative reduction to the `extracted` array.
  5) By default, set `selected_percent` to this relative reduction,
     unless there is a clearly more important A1c reduction value for the DRUG_HINT.
...
- If multiple timepoints are present, pick the one most clearly tied to the primary outcome
  (prefer 12 months/T12 over 6 months/T6 if ambiguous).
