...
def process_df(df, text_col):
    ...
    for idx, row in df.iterrows():
        ...
        # Additional rule:
        # - Keep numbers < 8
        # - Allow numbers >= 8 ONLY if it's an explicit from-to reduction (e.g., "from 9.1% to 7.8%")
        def _allowed(m):
            t = (m.get('type') or '').lower()
            if 'from-to' in t:  # allow explicit from-to phrases regardless of value
                return True
            # Otherwise, any numeric value < 8 passes
            ...
            for v in vals:
                try:
                    ...
                    pass
            # Also permit if computed reduction_pp < 8 (for safety)
